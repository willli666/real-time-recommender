...
class CounterRecommender(storage: CassandraStorage) {
  ...
  def checkUserViews(userId: String, item: ViewItem): Unit = {
    ...
    Try(Await.result(userViews, 3.second)) match {
      case Failure(msg) => println(msg)
      case Success(None) => saveNewUserViews(userId, item)
      case Success(Some(views)) => updateUserViews(views, item)
    }
  ...
  def saveNewUserViews(userId: String, item: ViewItem): Unit = {
    Await.result(storage.userViews.store(UserViews(userId, item :: Nil)), 3.second)
  }
  ...
  def updateUserViews(lastViews: UserViews, newItem: ViewItem): Unit = {
    if (lastViews.views.exists(_.itemId == newItem.itemId)) {
      ...
      Await.result(storage.userViews.store(UserViews(lastViews.userId, newItem :: updatedViews)), 3.second)
    }
    else {
      Await.result(storage.userViews.store(UserViews(lastViews.userId, (newItem :: lastViews.views).take(Config.RECENT_VIEWS_LIST_SIZE))),3.second)
    }
  }

  def checkBestSellers(itemId: String): Unit = {
    val bestSeller = storage.bestSellersIndex.getById(itemId)
    Try(Await.result(bestSeller, 3.second)) match {
      case Failure(msg) => println(msg)
  ...
  def saveNewBestSeller(itemId: String): Unit = {
    Await.result(storage.bestSellers.store(BestSeller("bestseller", itemId, 1)),3.second)
    Await.result(storage.bestSellersIndex.incrementCount(itemId),3.second)
  }

  def updateBestSeller(bestSeller: BestSellerIndex): Unit = {
    Await.result(storage.bestSellers.deleteRow(BestSeller("bestseller", bestSeller.itemId, bestSeller.score)),3.second)
    Await.result(storage.bestSellers.store(BestSeller("bestseller", bestSeller.itemId, bestSeller.score + 1)),3.second)
    Await.result(storage.bestSellersIndex.incrementCount(bestSeller.itemId),3.second)
  }
